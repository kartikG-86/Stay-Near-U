...
const NewPg = () => {
    ...
    const { pg, isLoading, error } = useSelector(
        (state) => state.pg
    );
    ...
    return(
        ...
        >
        <div>
            <Typography component="h1" variant="h5" align= "center" fontSize={35} padding={3}>
                Fill the PG Details
            </Typography>
            <TextField fullWidth
            {...register("pgName", { required: true })}
            id = "pgName"
            label="Pg Name" variant="filled" color="success" margin="normal"
        ...
        <div>
            ...
            id = "ownerName"
            label = "Owner Name" variant="filled" color="success" margin="normal"
        ...
        <div>
            <TextField id="contactNumber" fullWidth inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }}
            {...register("contactNumber", { required: true })} 
            label = "Contact Number" variant="filled" color="success" margin="normal"
        ...
        <div>
            {pricesFields.map((field, index) => (
                <div key={index}>
                    <TextField id="price" inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }}
                        value={field}
                        onChange={(e) => handlepricesFieldChange(index, e.target.value)}
                        required
                        label= "Price"
        ...
        <div>
            {facFields.map((field, index) => (
                <div key={index}>
                    <TextField
                        value={field}
                        onChange={(e) => handlefacFieldChange(index, e.target.value)}
                        required
                        label= "Facility"
                        variant="filled" color="success"
        ...
        <div>
            ...
            <label htmlFor="semi-furnished">
                <input type="radio" name="semi-furnished" id="semi-furnished" 
                {...register("furnishing")} />
                Semi-Furnished
            </label>
            <label htmlFor="furnishing">
                <input type="radio" name="non-furnished" id="non-furnished" 
                {...register("furnishing")} />
                Non-Furnished
            </label>
        </div>
        ...
        <div>
            <TextField fullWidth name="noticePriod" id="noticePriod" inputProps={{ inputMode: 'numeric', pattern: '[0-9]*' }}
        ...
        <div>
            ...
            <label htmlFor="singleRoom">
                <input type="radio" name="singleRoom" id="singleRoom"
                {...register("occupancyType")} />
                Single Room
            </label>
            <label htmlFor="doubleSharing">
                <input type="radio" name="doubleSharing" id="doubleSharing"
                {...register("occupancyType")} />
                Double Sharing Room
            </label>
            <label htmlFor="tripleSharing">
                <input type="radio" name="tripleSharing" id="tripleSharing"
                {...register("occupancyType")} />
                Triple Sharing Room
            </label>
        </div>
        ...
        <div>
            ...
            <label htmlFor="boys">
                <input type="radio" name="Boys" id="boys" required
                {...register("tenantType", {required: true})} />
                Boy
            </label>
            <label htmlFor="girls">
                <input type="radio" name="girls" id="girls" required
                {...register("tenantType", {required: true})} />
                Girl
            </label>
            <label htmlFor="others">
                <input type="radio" name="others" id="others" required
                {...register("tenantType", {required: true})} />
                Others
            </label>
        ...
        <div>
            <label htmlFor="pgImages">
                PG Images
            </label>
            <input type="file" name="pgImg" id="pgImg" accept="image/*" multiple
            ...
            onInput={(e) => {
                if (e.target.files && e.target.files.length > 0) {
                  ...
                  reader.onload = (event) => {
                    setpgImgPreview(event.target.result);
                  };
            ...
            hidden={pgImgPreview === "" ? true : false}
            />
        </div>
